...
class MockWorkoutRepository implements WorkoutRepository {
  final Map<String, WorkoutPlan> _plans = {
    'p1': WorkoutPlan(id: 'p1', name: 'Push Day', exercises: [
      const Exercise(
          id: 'e1', name: 'Supino reto', muscleGroup: 'Peito', details: '4x8'),
      const Exercise(
          id: 'e2',
    ...
    'p2': WorkoutPlan(id: 'p2', name: 'Pull Day', exercises: [
      ...
      const Exercise(
          ...
          name: 'Barra fixa',
          muscleGroup: 'Costas',
          details: '3x até falha'),
    ]),
  };

  final List<Exercise> _catalog = const [
    Exercise(
        id: 'c1', name: 'Rosca Scott', muscleGroup: 'Bíceps', details: '3x10'),
    Exercise(
        id: 'c2',
  ...
  Future<void> removeExercise(String planId, String exerciseId) async {
    final p = _plans[planId];
    if (p == null) return;
    _plans[planId] = p.copyWith(
        exercises: p.exercises.where((e) => e.id != exerciseId).toList());
  }
